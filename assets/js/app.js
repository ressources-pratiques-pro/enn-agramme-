document.addEventListener("DOMContentLoaded", () => {
  ...
  function accentForType(t){
    const g = groupOf(t);
    if(g === "instinctif") return "#fbbf24";
    if(g === "emotionnel") return "#f472b6";
    return "#60a5fa";
  }
  ...
  async function loadData(){
    const res = await fetch("assets/data/types.json", { cache: "no-store" });
  ...
  window.selectType = function selectType(t){
    ...
    openFull.setAttribute("href", `profils/type${t}.html`);
  ...
  document.getElementById("btnReset").addEventListener("click", ()=>{
    ...
    document.documentElement.style.setProperty("--accent", "rgba(255,255,255,.55)");
